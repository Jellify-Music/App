...
function BlurredBackground({
	...
	// Get artwork URL for fallback when blurhash is not available (e.g., Navidrome)
	...
	return (
		<ZStack flex={1} width={width} height={height}>
			<Animated.View
				...
				entering={FadeIn}
				exiting={FadeOut}
				key={`${nowPlaying?.item?.AlbumId ?? nowPlaying?.item?.Id ?? 'default'}-blurred-background`}
			>
				{blurhash ? (
					<Blurhash blurhash={blurhash} style={blurhashStyle} />
				) : artworkUrl ? (
					<Image
